---
parent: QmbRgevBjpHv8xeqhRkzLYVmg5wY8YYFWFNTjLNK8Urr7y
---

# Cyberlink and cybergraph

A *cyberparticle*, called just a *particle* below, is a unit of data with an address of its own — a CID (content identifier): an IPFS hash that uniquely identifies the content of a file (regardless of its name). For the purposes of the journal, particles are text only: human-readable text or machine-readable data structures serialized in JSON. Particles are connected by cyberlinks.
...
`cid_from` and `cid_to` are the CIDs of the two particles being connected. The pair `cid_from → cid_to` is an *edge* of the cybergraph, directed from `cid_from` to `cid_to`; the particles are its vertices. The `neuron` field is the CID of the neuron (Qm3), so the neuron can be referenced like any particle, and becomes a particle when its CID stands in an edge. The exact encoding is fixed in the appendix "Cybergraph format".
