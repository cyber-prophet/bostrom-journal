...
Cybergraph is a data structure made of cyberlinks, and each cyberlink is a record signed by a neuron, of the form:
...
`cid_from` and `cid_to` are the CIDs of the two particles being connected. The pair `cid_from → cid_to` is an *edge* of the cybergraph, directed from `cid_from` to `cid_to`; the particles are its vertices. The `neuron` field is the CID of the neuron (Qm3), so the neuron can be referenced like any particle, and becomes a particle by the same rule — when its CID stands in an edge. The exact encoding is fixed in the appendix "Cybergraph format".
