...
### Cybergraph [~](particles/QmbRgevBjpHv8xeqhRkzLYVmg5wY8YYFWFNTjLNK8Urr7y.md)
...
### Novelty [~](particles/QmeP7Z5vqZakmtftDfbsyqdq3FvGK3LJyr7JX9E4Ai5ifE.md)
...
### Appendix: Cybergraph format [~](particles/QmYcCP7x2kDQ466fmLMiPToDpUsXYDRDqF9hRvqQ5D4LUb.md)
...
`hash(x, y, …)` is always the CID of the string of arguments joined with commas; the result is a derived CID. Two special cases: the edge address — `hash(cid_from, cid_to)`; the cyberlink's derived CID — `hash(cid_from, cid_to, neuron)`. Joining with commas is one particular choice of encoding: the arguments can be encoded otherwise — for example, by a canonical serialization of the structure into JSON. What does not change: the encoding must be one and the same for everyone — otherwise, as with two encodings of the neuron above, the same act gets different derived CIDs.
