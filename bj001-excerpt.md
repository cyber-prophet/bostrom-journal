...
Приставка кибер- отмечает наш термин киберграфа и называется один раз, при введении; дальше по тексту — короткая форма. У основателя приставки нет — particle, neuron, — но namespace вынесен в путь его вики: `cybics/crystal/particle`, `cybics/crystal/neuron`. Приставка вносит тот же namespace в слово, поэтому расхождение только в форме записи.
